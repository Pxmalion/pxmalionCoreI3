...
#ifdef EEPROM_CUSTOM_FIL_FLAG

bool MarlinSettings::file_runout_detect;

bool  MarlinSettings::load_file_runout_state(void){
	return file_runout_detect;
}

void  MarlinSettings::store_file_runout_state_no_save(char new_flag){
	file_runout_detect = (new_flag!=0)?1:0;
}

void  MarlinSettings::file_runout_init(){
	
	int p =  EEPROM_CUSTOM_FIL_FLAG;
	uint8_t value = 0;
	uint16_t crc = 0;
	read_data(p,&value,1,&crc);
	
	if(value){
		file_runout_detect = true;
		}
	else
		file_runout_detect = 0;
}
void  MarlinSettings::store_file_runout_state(char new_flag){

	int p =  EEPROM_CUSTOM_FIL_FLAG;
	uint8_t value = new_flag;
	uint16_t crc = 0;
//	void MarlinSettings::write_data(int &pos, const uint8_t *value, uint16_t size, uint16_t *crc) {
	write_data(p,&value,1,&crc);
	file_runout_detect = (new_flag!=0)?1:0;
