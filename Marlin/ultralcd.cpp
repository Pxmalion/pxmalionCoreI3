...
void _moving_home()    { enqueue_and_echo_commands_P(PSTR("G28")); } // move all axes home and wait
...
#define _MENU_ITEM_PART_FUNC_1(TYPE,FUNC, ...) \
    if (_menuLineNr == _thisItemNr) { \
      if (lcd_clicked && encoderLine == _thisItemNr) {\
	  	FUNC();

#define MENU_ITEM_FUNC(TYPE, LABEL,FUNC,...) do { \
      _skipStatic = false; \
      _MENU_ITEM_PART_FUNC_1(TYPE, FUNC,## __VA_ARGS__); \
      _MENU_ITEM_PART_2(TYPE, LABEL, ## __VA_ARGS__); \
    }while(0)
  ...
  // updata lcd menu all time
  bool lcd_keep_update_alltime = 0;
  // Button States
...
#ifdef LCD_AUTO_BED_LEVELING_UBL


#ifdef PXMALION_DPRINTER

static void lcd_prepare_advance(void);

static int user_cmd = 0;
#define _REFRESHLCD_MENU() do{ \
								refresh_cmd_timeout(); \
								lcdDrawUpdate = LCDVIEW_REDRAW_NOW; \
							}while(0)
							
static void lcd_implementation_menu(const char *l1,
			const char *l2,const char *l3,const char *l4) {
	  // Line 2   
	if(l1){
		lcd.setCursor(_GET_CENTER_Y_POINT(LCD_WIDTH,l1), 0);
		lcd_print(l1);
	}
	if(l2){
		lcd.setCursor(_GET_CENTER_Y_POINT(LCD_WIDTH,l2), 1);
		lcd_print(l2);
	}
	if(l3){
		lcd.setCursor(_GET_CENTER_Y_POINT(LCD_WIDTH,l3), 2);
		lcd_print(l3);
	}
	if(l4){
		lcd.setCursor(_GET_CENTER_Y_POINT(LCD_WIDTH,l4), 3);
		lcd_print(l4);
	}

}
// get encoder input 
static void _get_encoder_input(int *target,const int multiple,const int max,const int min){
	encoderRateMultiplierEnabled = false;
	refresh_cmd_timeout();
	// Get the new position

	*target += ((int)encoderPosition * multiple);
...
*   -------------------------
*/
static void lcd_implementation_auto_bed_leveling() {
	  // Line 2   
	//lcd.setCursor(_GET_CENTER_Y_POINT(LCD_WIDTH,MSG_AUTO_LEVEL_2), 1);
	//lcd_printPGM(PSTR(MSG_AUTO_LEVEL_2));
}


static void lcd_implementation_auto_bed_leveling_home() {
		
	  // Line 1   
	lcd.setCursor(_GET_CENTER_Y_POINT(LCD_WIDTH,MSG_AUTO_LEVEL_HOMING_1), 0);
	lcd_printPGM(PSTR(MSG_AUTO_LEVEL_HOMING_1));
	  // Line 2   
	lcd.setCursor(_GET_CENTER_Y_POINT(LCD_WIDTH,MSG_AUTO_LEVEL_HOMING_2), 1);
	lcd_printPGM(PSTR(MSG_AUTO_LEVEL_HOMING_2));
	  // Line 4   
	lcd.setCursor(_GET_CENTER_Y_POINT(LCD_WIDTH,MSG_AUTO_LEVEL_HOMING_4), 3);
	lcd_printPGM(PSTR(MSG_AUTO_LEVEL_HOMING_4));
}


void _lcd_bed_leveling_g29ing(void){
	
	_REFRESHLCD_MENU();
	defer_return_to_status = true;
		/****
	**	auto bed leveing home menu
	*	-------------------------
	*	|	Probing 15 point |
	*	|	Please Wait ...  |
	*	|	                 |
	...
	*/
	if(lcdDrawUpdate)
		lcd_implementation_menu(MSG_AUTO_DO_BED_LEVEING_1,MSG_AUTO_DO_BED_LEVEING_2,NULL,MSG_AUTO_DO_BED_LEVEING_4);
	if(!planner.movesplanned() ){
		set_bed_leveling_enabled(true);
  		settings.save();
		settings.load();
		defer_return_to_status = false;
		lcd_goto_screen(lcd_prepare_menu); 
	}
}	

void _lcd_bed_leveling_proble_test(void){
	
	defer_return_to_status = true;
	lcd_keep_update_alltime = true;
	_REFRESHLCD_MENU();
/****
**	auto bed leveing home menu
*	-------------------------
...
*/
		  ...
		  if(lcdDrawUpdate){
			  ...
			  if(READ(Z_MIN_PROBE_PIN)^Z_MIN_PROBE_ENDSTOP_INVERTING){
			  		...
			  		lcd_implementation_clear();
				  	lcd_implementation_menu(MSG_AUTO_LEVEL_PROBLE_TESTING_1,
						  MSG_AUTO_LEVEL_PROBLE_TESTING_2,MSG_ENDSTOP_HIT,MSG_AUTO_LEVEL_PROBLE_TESTING_4);
			  }else{
			  	
			  	  	lcd_implementation_clear();
				  	lcd_implementation_menu(MSG_AUTO_LEVEL_PROBLE_TESTING_1,
						  MSG_AUTO_LEVEL_PROBLE_TESTING_2,MSG_ENDSTOP_OPEN,MSG_AUTO_LEVEL_PROBLE_TESTING_4);
			  }			  
		}
#endif
	if(lcd_clicked){
		
		lcd_keep_update_alltime = false;
		enqueue_and_echo_commands_P(PSTR("G29 P1 T")); 
		lcd_goto_screen(_lcd_bed_leveling_g29ing);
	}
}

void _lcd_bed_leveling_proble_isntall(void){
	
	_REFRESHLCD_MENU();
	
	defer_return_to_status = true;
	/****
	**	auto bed leveing install probe menu 
	*	-------------------------
	*	|					  |
	*	|	Install Probe	  |
	*	|	Under Nozzel  |
	...
	*/
	...
	if(lcdDrawUpdate)
		lcd_implementation_menu(MSG_AUTO_LEVEL_PROBLE_INSTALL_1,
			MSG_AUTO_LEVEL_PROBLE_INSTALL_2,NULL,MSG_AUTO_LEVEL_PROBLE_INSTALL_4);
	if(lcd_clicked){
		//enqueue_and_echo_commands_P(PSTR("G29 P1 T")); 
		lcd_goto_screen(_lcd_bed_leveling_proble_test);
	}
}

/**
 * auto UBL System submenu  
 *
 * << Auto Bed Leveling
 * <<<< waitting for auto home
 * <<<<<< install probe
 **<<<<<<<<< auto bed leveling
 */
void _lcd_auto_bed_leveing(void){
	...
	SERIAL_PROTOCOLPGM("_lcd_auto_bed_leveing ...");
	// waitting  G28 end 
	_REFRESHLCD_MENU();
	/****
**	auto bed leveing home menu
...
*/
	if(lcdDrawUpdate)
		lcd_implementation_menu(MSG_AUTO_LEVEL_HOMING_1,MSG_AUTO_LEVEL_HOMING_2,NULL,MSG_AUTO_LEVEL_HOMING_4);
		lcd_implementation_auto_bed_leveling_home();

	if( !planner.movesplanned() ){ //  if g28 end 
		// init eeprom 
		// freash 
		settings.reset();
		settings.save();
		...
		// next lcd menu.
		//up Z axix
		enqueue_and_echo_commands_P(PSTR("G1 Z50")); 
		lcd_goto_screen(_lcd_bed_leveling_proble_isntall);
	}
	if(lcd_clicked){
		return lcd_goto_screen(lcd_prepare_menu); 
		
	}
	// raish z axix 50 mm	
}

#define _FILEMENT_Z_SPEED (1000)
#define _FILA_TMP_INIT (250)
#define _MOVE_Z_UP_100MM_LEN (100)
#define _FILEMENT_INSERT_SPEED (120) // MM/S
#define _FILEMENT_OUT_SPEED (300)  // MM/S

#define _FILEMENT_LOAD_INSERT_LEN	(150) // MM
#define _FILEMENT_UNLOAD_INSERT_LEN (10)
#define _FILEMENT_UNLOAD_OUT_LEN (150)


typedef void (*user_menuFunc_t)(void);

static void lcd_prepare_filament_menu();

/****
**	load Nozzle Heating menu
*   -------------------------
*	|     Nozzle Heating    |
*	|     TEMP:175/250℃      |
*	| (Rotate Adjust temp） |
...
*   -------------------------
*/
static void lcd_implementation_getNozzle_temp_screen() {

  // Line 1 centered "Nozzle Heating"  
	lcd.setCursor(_GET_CENTER_Y_POINT(LCD_WIDTH,MSG_LOAD_FILAMENT_L1), 0);
	lcd_printPGM(PSTR(MSG_LOAD_FILAMENT_L1));
	
  // Line 2  centered "TEMP:23/230℃"

	lcd.setCursor(_GET_CENTER_Y_POINT_ADD(LCD_WIDTH,MSG_LOAD_FILAMENT_L2,9), 1);
	lcd_printPGM(PSTR(MSG_LOAD_FILAMENT_L2));
	lcd.print(itostr3( thermalManager.degHotend(0) + 0.5));
	lcd.print('/');
	lcd.print(itostr3left( thermalManager.degTargetHotend(0) + 0.5));
	lcd_printPGM(PSTR("\x01" "C"));

// Line 3  centered "(Rotate Adjust temp)"
	lcd.setCursor(_GET_CENTER_Y_POINT(LCD_WIDTH,MSG_LOAD_FILAMENT_L3), 2);
	lcd_printPGM(PSTR(MSG_LOAD_FILAMENT_L3));
	
// Line 4  centered "( Click to back)"
	lcd.setCursor(_GET_CENTER_Y_POINT(LCD_WIDTH,MSG_LOAD_FILAMENT_L4), 3);
	lcd_printPGM(PSTR(MSG_LOAD_FILAMENT_L4));
}

static void _backHome_heading(void){
	// set target temp
	thermalManager.setTargetHotend(min(heater_maxtemp[0], _FILA_TMP_INIT), 0);
	lcd_implementation_clear();
	_moving_home();
	...
	// speed_s in mm/s
static void _move_to_xyze(float px,float py,float pz,float pe,float speed_s){

	if(px)
		current_position[X_AXIS] += px;
	if(py)
		current_position[Y_AXIS] += (py);
	if(pz)
		current_position[Z_AXIS] += (pz);
	if(pe)
		current_position[E_AXIS] += (pe);
	planner.buffer_line_kinematic(current_position,speed_s, active_extruder);	
}

/**
 *
 * "Prepare" > "Change flament" > "Load filament" submenu
 *
 */
static void lcd_heating_filement_menu(int cmd,user_menuFunc_t lcd_show,screenFunc_t next_menu) {
	...
	int target_tmp = thermalManager.degTargetHotend(0);
	_get_encoder_input(&target_tmp,1,250,175);
	

	thermalManager.setTargetHotend(target_tmp, 0);
	...
	if ( thermalManager.degHotend(0) >= thermalManager.degTargetHotend(0) )
		...
		// upper z axix
		// Limit only when trying to move towards the limit
      	//NOMORE(current_position[Z_AXIS], _MOVE_Z_UP_100MM_LEN);
		//manual_move_to_current(Z_AXIS);
		_move_to_xyze(0,0,_MOVE_Z_UP_100MM_LEN,0,_FILEMENT_Z_SPEED);
		
		//enqueue_and_echo_commands_P(PSTR("G1 Z100 F1000")); 
		lcd_keep_update_alltime = false;
		
		user_cmd = cmd;
		
		lcd_implementation_clear();
		lcd_goto_screen(next_menu);
	}
		
	// show curren lcd menu
	defer_return_to_status = true;
	lcd_keep_update_alltime = true;
	_REFRESHLCD_MENU();
...
static void lcd_prepare_filement_chanage_finish(){

    if (lcd_clicked) { 
		_moving_home();
		return lcd_goto_screen(lcd_prepare_filament_menu); 
	}
	//show lcd	
	_REFRESHLCD_MENU();
	defer_return_to_status = true;
	if(lcdDrawUpdate){
		if(user_cmd == USER_CMD_CONFIG_UNLOAD_DONE)
			lcd_implementation_menu(MSG_Unload_FILAMENT_DONE_L1,MSG_Unload_FILAMENT_DONE_L2,0,0);
		else 
			lcd_implementation_menu(MSG_insert_FILAMENT_DONE_L1,MSG_insert_FILAMENT_DONE_L2,0,0);
	}
	
}

#define _FILA_INSERT_SPEED (1)  // 1.5MM/S   // 进料界面，进料速度
#define _FILA_INSERT_LEN (150)  // 150 MM    // 进料界面，进料长度
#define _FILA_UNLOAD_IN_SPEED (1) //退料 界面 进料 速度
#define _FILA_UNLOAD_IN_LEN    (10) // 退料  界面 进料长度
#define _FILA_UNLOAD_OUT_SPEED (4) //退料界面，退料速度
#define _FILA_UNLOAD_OUT_LEN (150) //退料界面，退料长度


#define _ADJUST_POSITION_XY  (110)
#define _ADJUST_POSITION_Z_1   (20)
#define _ADJUST_POSITION_Z_2   (190)
#define _ADJUST_POSITION_Z   (190)

#define _ADJUST_SPEED		 (200)
void user_config_manage(void){

		// upper z axis

	switch(user_cmd){
		case USER_CMD_CONFIG_LOAD:
			
			stepper.synchronize();
//			enqueue_and_echo_commands_P(PSTR("G92 E0"));
//		    stepper.synchronize();
			
//			enqueue_and_echo_commands_P(PSTR("G1 F60")); 
			// insert 150mm in speed  120mm/min 
//			enqueue_and_echo_commands_P(PSTR("G1 E150")); 
			
			_move_to_xyze(0,0,0,_FILA_INSERT_LEN,_FILA_INSERT_SPEED);
			stepper.synchronize();
			user_cmd = USER_CMD_CONFIG_LOAD_DONE;

			break;
		case USER_CMD_CONFIG_UNLOAD:
#if 0			
			stepper.synchronize();
			enqueue_and_echo_commands_P(PSTR("G92 E0"));
		    stepper.synchronize();
			// insert 10mm in speed  120mm/min  G92 E0
			enqueue_and_echo_commands_P(PSTR("G1 E10 F120")); 
			stepper.synchronize();
			// uninsert 150mm in 300m/min
			enqueue_and_echo_commands_P(PSTR("G1 E-150 F300"));
#endif			
			stepper.synchronize();
			_move_to_xyze(0,0,0,_FILA_UNLOAD_IN_LEN,_FILA_UNLOAD_IN_SPEED);
			stepper.synchronize();
			_move_to_xyze(0,0,0,-_FILA_UNLOAD_OUT_LEN,_FILA_UNLOAD_OUT_SPEED);
			stepper.synchronize();
			user_cmd = USER_CMD_CONFIG_UNLOAD_DONE;
			break;
		case USER_CMD_TEST_ADJUSTING_Z_HOME:
			
			home_all_axes();
			stepper.synchronize();
			_move_to_xyze(_ADJUST_POSITION_XY,_ADJUST_POSITION_XY,_ADJUST_POSITION_Z_1,0,_ADJUST_SPEED);
			stepper.synchronize();
			_move_to_xyze(0,0,(_ADJUST_POSITION_Z-_ADJUST_POSITION_Z_1 +10),0,_ADJUST_SPEED);
			stepper.synchronize();
			_move_to_xyze(0,0,-_ADJUST_POSITION_Z,0,_ADJUST_SPEED);
			stepper.synchronize();
			_move_to_xyze(0,0,_ADJUST_POSITION_Z,0,_ADJUST_SPEED);
			stepper.synchronize();
			_move_to_xyze(0,0,-_ADJUST_POSITION_Z,0,_ADJUST_SPEED);
			stepper.synchronize();
			user_cmd = USER_CMD_TEST_ADJUSTING_Z_DONE;

			break;
	}

}
static void lcd_prepare_filement_change(void){
	...
	lcdDrawUpdate = LCDVIEW_REDRAW_NOW;
	if(lcdDrawUpdate){
		if(user_cmd <= USER_CMD_CONFIG_LOAD_DONE){  // load
		
			lcd_implementation_clear();
			lcd_implementation_menu(MSG_insert_FILAMENT_L1,MSG_insert_FILAMENT_L2,
				MSG_insert_FILAMENT_L3,MSG_insert_FILAMENT_L4);	
		}else{
			
			lcd_implementation_clear();
			lcd_implementation_menu(MSG_insert_FILAMENT_L1,MSG_UNload_FILAMENT_L2,
						MSG_unload_FILAMENT_L3,MSG_insert_FILAMENT_L4); 
			}
			...
			// upper z axis
	stepper.synchronize();
	if(user_cmd == USER_CMD_CONFIG_LOAD){

		// insert 150mm in speed  120mm/min 
		enqueue_and_echo_commands_P(PSTR("G1 E150 F120")); 
		stepper.synchronize();
		user_cmd = USER_CMD_CONFIG_LOAD_DONE;
	}else{
		// insert 10mm in speed  120mm/min 
		enqueue_and_echo_commands_P(PSTR("G1 E10 F120")); 
		stepper.synchronize();
		// uninsert 150mm in 300m/min
		enqueue_and_echo_commands_P(PSTR("G1 E-150 F300"));
	...
	if( !planner.movesplanned()  && 
		(user_cmd == USER_CMD_CONFIG_LOAD_DONE || user_cmd == USER_CMD_CONFIG_UNLOAD_DONE)){
		...
		thermalManager.disable_all_heaters();
		lcd_keep_update_alltime = false;
		lcd_goto_screen(lcd_prepare_filement_chanage_finish);
	}

}

static void lcd_prepare_filement_loading(void){

	lcd_prepare_filement_change();

}

static void lcd_prepare_filement_unloading(void){
 ...
 *
 */
static void lcd_prepare_filement_load_menu(void) {
	lcd_heating_filement_menu(USER_CMD_CONFIG_LOAD,
		lcd_implementation_getNozzle_temp_screen,lcd_prepare_filement_loading);
}
/**
 *
 * "Prepare" > "Change flament" > "Unload filament" submenu
 *
 */
static void lcd_prepare_filement_unload_menu(void) {
	lcd_heating_filement_menu(USER_CMD_CONFIG_UNLOAD,
		lcd_implementation_getNozzle_temp_screen,lcd_prepare_filement_unloading);
}

/**
 * "Prepare" > "Change flament" submenu
 */
static void lcd_prepare_filament_menu() {
  START_MENU();
  MENU_ITEM(back, MSG_MAIN, lcd_prepare_menu);
  MENU_ITEM_FUNC(submenu, MSG_LOAD_FILAMENT,_backHome_heading,lcd_prepare_filement_load_menu);
  MENU_ITEM_FUNC(submenu, MSG_UNLOAD_FILAMENT,_backHome_heading, lcd_prepare_filement_unload_menu);
  END_MENU();
}
static void lcd_prepare_adjust_z_axis_home(void){
	//_moving_home();
 	user_cmd = USER_CMD_TEST_ADJUSTING_Z_HOME;
}
#define _LCD_ADJUST_SPEED	(300)  // MM/S
static void lcd_prepare_adjust_z_axis_menu(void){
	...
	defer_return_to_status = true;
	if(lcdDrawUpdate)
		lcd_implementation_menu(MSG_ADJUST_Z_AXIS_L1,MSG_ADJUST_Z_AXIS_L2,
						MSG_ADJUST_Z_AXIS_L3,MSG_ADJUST_Z_AXIS_L4); 
	
	if(  user_cmd == USER_CMD_TEST_ADJUSTING_Z_DONE){
...
*   -------------------------
*/
static void lcd_implementation_sensorState_screen() {

  // Line 1 centered "Nozzle Heating"  
	lcd.setCursor(0, 0);
	lcd_printPGM(PSTR(MSG_SENSOR_INFO_HTEMP));
	lcd.print(itostr3( thermalManager.degHotend(0) + 0.5));
	lcd.print(' ');
	lcd_printPGM(PSTR(MSG_SENSOR_INFO_BTEMP));
	lcd.print( itostr3( thermalManager.degBed()  + 0.5));
    
	lcd.setCursor(0, 1);
	lcd_printPGM(PSTR(MSG_SENSOR_INFO_XIM));
	...
	lcd_printPGM(PSTR(MSG_SENSOR_INFO_YIM));
	...
	lcd_printPGM(PSTR(MSG_SENSOR_INFO_ZIM));
	...
	lcd_printPGM(PSTR(MSG_SENSOR_INFO_PRO));
	if(READ(Z_MIN_PROBE_PIN)^Z_MIN_PROBE_ENDSTOP_INVERTING)
		lcd.print('1');
	else
		lcd.print('0');
	...
	lcd_printPGM(PSTR(MSG_SENSOR_INFO_FIL));
...
//extern bool man_disable_filament_ran_out ;

static void lcd_prepare_filament_ranout_disable(void){
	
	settings.store_file_runout_state(false);
	lcd_keep_update_alltime = true;
	//man_disable_filament_ran_out = true;
}
static void lcd_prepare_filament_ranout_enable(void){
	lcd_keep_update_alltime = true;
	settings.store_file_runout_state(true);
	//man_disable_filament_ran_out = false;
}


static void lcd_prepare_sensor_info(void){
	...
	defer_return_to_status = true;
	if(lcdDrawUpdate)
		lcd_implementation_sensorState_screen();
	
	if (lcd_clicked) { 
		lcd_keep_update_alltime = false;
		return  lcd_goto_screen(lcd_info_menu);
	}
...
END_MENU();
  ...
  void lcd_prepare_menu() {
    ...
    //
    MENU_ITEM(gcode, MSG_AUTO_HOME, PSTR("G28"));
	MENU_ITEM_FUNC(submenu,MSG_AUTO_BED_LEVEING,_moving_home,_lcd_auto_bed_leveing);
#if 0
//#if ENABLED(ADVANCED_PAUSE_FEATURE)
    if (!thermalManager.tooColdToExtrude(active_extruder))
        MENU_ITEM(function, MSG_FILAMENTCHANGE, lcd_enqueue_filament_change);
	//else
	// 	MENU_ITEM(submenu,MSG_FILAMENTCHANGE,lcd_prepare_filament_menu);
#endif
	MENU_ITEM(submenu,MSG_FILAMENTCHANGE,lcd_prepare_filament_menu);

	MENU_ITEM_FUNC(submenu,MSG_ADJUST_Z,lcd_prepare_adjust_z_axis_home,lcd_prepare_adjust_z_axis_menu);
	//MENU_ITEM(submenu, MSG_AUTO_BED_LEVEING, _lcd_auto_bed_leveing);	 
    //
    // Move Axis
    #if ENABLED(DELTA)
	...
	if( settings.load_file_runout_state())
	  MENU_ITEM(function, MSG_DISABLE_FIL_RUNOUT, lcd_prepare_filament_ranout_disable);
	else 
	  MENU_ITEM(function, MSG_ENABLE_FIL_RUNOUT, lcd_prepare_filament_ranout_enable);

	// Disable the leveling matrix before homing
  #if ENABLED(AUTO_BED_LEVELING_UBL)
	if(leveling_is_active())
		MENU_ITEM(gcode, MSG_LEVELING_DISABLE, PSTR("M420 S0"));
	else
		MENU_ITEM(gcode, MSG_LEVELING_ACTIVE, PSTR("M420 S1"));
  #endif
    //
    // Disable Steppers
    //
	...
	MENU_ITEM(submenu, MSG_PREPARE_ADVANCE, lcd_prepare_advance);
    END_MENU();
  }
  ...
  static void lcd_control_advance(void){
  ...
  void lcd_control_menu() {
    ...
    #endif
	MENU_ITEM(submenu, MSG_PREPARE_ADVANCE, lcd_control_advance);
   ...
   */
    ...
    void lcd_info_menu() {
	  ...
	  MENU_ITEM(submenu, MSG_SENSOR_INFO_MENU, lcd_prepare_sensor_info);
   ...
   */
    ...
    void lcd_advanced_pause_resume_print_no_fil_detect() {
    
	  settings.store_file_runout_state_no_save(0);
      advanced_pause_menu_response = ADVANCED_PAUSE_RESPONSE_RESUME_PRINT;
    ...
    void lcd_advanced_pause_option_menu() {
	  ...
	  // ADD by sky 
      MENU_ITEM(function, MSG_FILAMENT_CHANGE_OPTION_RESUME_NODETECT, lcd_advanced_pause_resume_print_no_fil_detect);
...
void lcd_update() {
  ...
  if (ELAPSED(ms, next_lcd_update_ms)
    ...
    next_lcd_update_ms = ms + LCD_UPDATE_INTERVAL;
      ...
      if ( lcd_keep_update_alltime || encoderPastThreshold || lcd_clicked) {
